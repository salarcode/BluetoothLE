...
public partial class MainPage : ContentPage
{
    ...
    {
        var access = await BlePermissionHelper.RequestAccessAsync(_adapter);
        PermissionStatusLabel.Text = access.DisplayStatus;
        UpdateStatus();

        if (!access.IsGranted)
        {
            FeedbackLabel.Text = access.Message;
            await DisplayAlert("BLE Access", access.Message, "OK");
        }
