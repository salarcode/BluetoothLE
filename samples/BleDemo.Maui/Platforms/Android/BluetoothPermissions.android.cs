			...
			get
			{
				...
				else
				{
					...
					if (sdk >= 29)
					{
						// TODO: 
						/*
						This adds ACCESS_BACKGROUND_LOCATION to the required permissions for SDK >= 29 whenever scan/connect/advertise is enabled. 
							Background location is a highly restricted permission and typically must be requested in a separate, explicit user flow (and only if the app truly scans in the background); 
							requesting it unconditionally here can cause permission denial and onboarding failures.
						*/
