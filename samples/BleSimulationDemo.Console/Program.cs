using BleDemo.Console.Simulation;
...
// ── Bootstrap ─────────────────────────────────────────────────────────────────
// In a real MAUI/Windows/Android app you would inject the platform adapter
// via DI (e.g. builder.UseBle() from Salar.BluetoothLE.Maui).  The console demo
// uses a self-contained SimulatedBleAdapter so it runs on any OS without
// real BLE hardware.
using var adapter = new SimulatedBleAdapter();
...
ConsoleUi.Dim("Adapter: SimulatedBleAdapter  (swap for AndroidBleAdapter / WindowsBleAdapter in production)");
    ...
    try
    {
        await ConsoleUi.SpinAsync("Connecting…", Task.Run(async () =>
        {
            device = await adapter.ConnectAsync(
                selected.Address,
                new ConnectionConfig { ConnectionTimeout = TimeSpan.FromSeconds(10) });
        }));
...
async Task DeviceMenuAsync(IBleDevice device, ScanResult? scanResult)
{
    while (true)
    {
        ...
        {
            case 1:
                await CheckSignalAsync(device, scanResult);
                break;
...
async Task CheckSignalAsync(IBleDevice device, ScanResult? lastScan)
{
    ConsoleUi.PrintHeader("Signal Strength");

    // For a real adapter you'd trigger a ReadRssi call; here we use the
    // simulated device's drifting RSSI value.
    if (device is SimulatedBleDevice sim)
    {
        int rssi = sim.Rssi;
        System.Console.Write("  RSSI: ");
    ...
    else if (lastScan is not null)
    {
        // Fallback: show last scanned RSSI
        System.Console.Write("  Last scanned RSSI: ");
        ConsoleUi.WriteLine(ConsoleUi.SignalBar(lastScan.Rssi), RssiColor(lastScan.Rssi));
    }
    else
    {
        ConsoleUi.Dim("  RSSI not available on this platform.");
    }
    ...
    else
    {
        foreach (var svc in services)
        {
            ...
            foreach (var ch in chars)
            {
                ...
                ConsoleUi.Dim($"      ◦ {ch.Uuid.ToString().ToUpperInvariant()}  [{flags}]");
            }
        }
    }
